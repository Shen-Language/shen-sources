...
-expand -in -column -columnspan -row -rowspan -sticky -weight -slant -tag roman italic scrollbar eot . tk.destroy
manager name parent toplevel height width ismapped pointerx pointery reqheight reqwidth rootx rooty
screenheight screenwidth viewable]
 ...
 (declare bindkey       [widget --> [event --> [[lazy A] --> [lazy A]]]])
 (declare wipe          [canvas --> [symbol --> symbol]])            )