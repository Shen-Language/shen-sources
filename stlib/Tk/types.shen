...
(datatype general
    ...
    (my-draw Canvas Shape Coor Options) : (instance Shape);

    if (element? Shape [line arc rectangle oval polygon text])
...
(datatype drawing
    ...
    Shape : shape;
    ______________________________
    -tag : (attribute Shape string);

    Shape : shape;
    ______________________________
    -tag : (attribute Shape symbol);

    Shape : shape;
    ______________________________
    -outline : (attribute Shape color);

    Shape : shape;
    ______________________________
    -fill : (attribute Shape color);

    ________________________________
    -arrow : (attribute line arrow);)

(datatype winfos

   if (element? X [exists height width ismapped pointerx
                   pointery reqheight reqwidth rootx rooty screenheight
                   screenwidth viewable x y])
   _________________________________
   X : (attribute winfo number);

   if (element? X [manager name parent toplevel])
   ______________________________________________
   X : (attribute winfo symbol);)
...
(define hex?
  D -> (element? D [($ "abcdefABCDEF1234567890")]))
...
(define event?
    X -> (and (symbol? X) (compile (fn <complex-event>) (explode X))))
  ...
  (defcc <events>
   ...
   (defcc <event>
      ...
      ($ Button-4) := skip;
      ($ Button-5) := skip;
      X := skip  where (element? X [($ abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ)]);)
