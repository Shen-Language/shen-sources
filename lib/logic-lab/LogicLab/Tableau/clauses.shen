...
(define amplify-clauses
  {number --> prop --> (list prop)}
   N P -> (elim-& (elim-all (amplify N (skolemise (prenex P))))))
   
(define amplify
  {number --> prop --> prop}
   1 P -> P
   N [all X P] -> (skolemise (prenex [[all X P] & (amplify (- N 1) (rectify [all X P]))]))
   _ P -> P)   
...
(define rectify
    {prop --> prop}
    [all X P] -> (let Y (gensym x)  [all Y (rectify (replace X Y P))])
    [exists X P] -> (let Y (gensym x)  [exists Y (rectify (replace X Y P))])
